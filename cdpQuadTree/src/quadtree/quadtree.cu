...
////////////////////////////////////////////////////////////////////////////////
// Build a quadtree level on the GPU.
//
// The algorithm works as follows. The host (CPU) launches n block of
...
__global__
void build_quadtree_level_kernel(Quadtree_node *nodes, Points *points, Parameters params, int nodes_offset = 0)
{
    ...
    // The current node.
    Quadtree_node &node = nodes[nodes_offset + blockIdx.x];

    //if (threadIdx.x == NUM_THREADS_PER_BLOCK-1)
    //    printf("Processing node %d, id %d\n", nodes_offset + blockIdx.x, node.id());
    ...
    for (int range_it = range_begin + lane_id ; __any_sync(__activemask(), range_it < range_end) ; range_it += warpSize)
    {
        ...
        int num_pts = __popc(__ballot_sync(__activemask(), is_active && p.x < center.x && p.y >= center.y));

        if (num_pts > 0 && lane_id == 0)
            s_num_pts[0][warp_id] += num_pts;

        // Count top-right points.
        num_pts = __popc(__ballot_sync(__activemask(), is_active && p.x >= center.x && p.y >= center.y));
...
#pragma unroll
        ...
        {
            int n = __shfl_up_sync(__activemask(), num_pts, offset, NUM_WARPS_PER_BLOCK);
    ...
    for (int range_it = range_begin + lane_id ; __any_sync(__activemask(), range_it < range_end) ; range_it += warpSize)
    {
        ...
        int vote = __ballot_sync(__activemask(), pred);
        ...
        vote = __ballot_sync(__activemask(), pred);
    ...
    if (threadIdx.x == NUM_THREADS_PER_BLOCK-1)
    {
        ...
        int child_offset = 4*node.id() + 1;

        // Set IDs.
        nodes[child_offset+0].set_id(child_offset + 0);
        nodes[child_offset+1].set_id(child_offset + 1);
        nodes[child_offset+2].set_id(child_offset + 2);
        nodes[child_offset+3].set_id(child_offset + 3);
        ...
        nodes[child_offset+0].set_bounding_box(p_min.x , center.y, center.x, p_max.y);    // Top-left.
        nodes[child_offset+1].set_bounding_box(center.x, center.y, p_max.x , p_max.y);    // Top-right.
        nodes[child_offset+2].set_bounding_box(p_min.x , p_min.y , center.x, center.y);   // Bottom-left.
        nodes[child_offset+3].set_bounding_box(center.x, p_min.y , p_max.x , center.y);   // Bottom-right.

        // Set the ranges of the children.
        nodes[child_offset+0].set_range(node.points_begin(),   s_num_pts[0][warp_id]);
        nodes[child_offset+1].set_range(s_num_pts[0][warp_id], s_num_pts[1][warp_id]);
        nodes[child_offset+2].set_range(s_num_pts[1][warp_id], s_num_pts[2][warp_id]);
        nodes[child_offset+3].set_range(s_num_pts[2][warp_id], s_num_pts[3][warp_id]);
    }
}
...
bool check_quadtree(const Quadtree_node *nodes, int idx, int num_pts, Points *pts, Parameters params)
{
    ...
    //std::cout << "Nodo " << idx << " id " << node.id() << " punti " << num_points << std::endl;

    // 1. Controlla che tutti i punti siano nel bounding box
    for (int it = node.points_begin(); it < node.points_end(); ++it) {
        if (it >= num_pts) return false;
        float2 p = pts->get_point(it);
        if (!bbox.contains(p)){
            std::cout << "Punto " << it << " fuori dal bounding box: " << p.x << ", " << p.y << std::endl;
            return false;
        }
    }

    // 2. Se è un nodo foglia, non ci sono figli -> terminazione
    if (params.depth >= params.max_depth || num_points <= params.min_points_per_node) {
        return true;
    }

    const int children_offset = 4 * idx + 1;
    // 3. Se è nodo interno, controlla i figli
    int num_points_in_children = 0;
    num_points_in_children += nodes[children_offset+0].num_points();
    num_points_in_children += nodes[children_offset+1].num_points();
    num_points_in_children += nodes[children_offset+2].num_points();
    num_points_in_children += nodes[children_offset+3].num_points();

    // Verifica coerenza tra padre e figli
    if (num_points_in_children != node.num_points()) {
        std::cout << "Nodo " << params.num_nodes_at_this_level / 4 + idx << " id " << node.id() << " incoerenza, figli " << num_points_in_children << std::endl;
        return false;
    }

    // Verifica ricorsiva dei figli
    Parameters child_params(params, true);
    return check_quadtree(nodes, children_offset+0, num_pts, pts, child_params) &&
           check_quadtree(nodes, children_offset+1, num_pts, pts, child_params) &&
           check_quadtree(nodes, children_offset+2, num_pts, pts, child_params) &&
           check_quadtree(nodes, children_offset+3, num_pts, pts, child_params);
}
...
bool cdpQuadtree(int warp_size)
{
    ...
    //std::cout << "Maximum number of nodes in the quadtree: " << max_nodes << std::endl;
    ...
    Parameters curr_params = params;
    int nodes_offset = 0;

    double t1 = omp_get_wtime();
    for (int level = 0; level <= max_depth; level++)
    {
        int num_nodes = curr_params.num_nodes_at_this_level;
        //printf("Building level %d with %d nodes, offset %d\n", level, num_nodes, nodes_offset);
        build_quadtree_level_kernel<NUM_THREADS_PER_BLOCK><<<num_nodes, NUM_THREADS_PER_BLOCK, smem_size>>>(nodes, points, curr_params, nodes_offset);
        checkCudaErrors(cudaGetLastError());
        checkCudaErrors(cudaDeviceSynchronize());
        nodes_offset += num_nodes; // Update the offset for the next level.
        curr_params = Parameters(curr_params, true); // Increment the parameters for the next level.
    }
    double t2 = omp_get_wtime();
    std::cout << "Quadtree built in: " << (t2 - t1) << " seconds." << std::endl;
